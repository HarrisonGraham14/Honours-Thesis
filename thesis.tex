...
This poses a particular challenge in \autoref{s-comp}, 
...
\begin{itemize}
    \item A completed draft of \autoref{s-forms} is included, where some of the basic definitions are established.
    Here, we establish that forms of a fixed discriminant can be partitioned into a finite set of equivalence classes.

    \item A bare-bones version of \autoref{s-comp} is included.
    ...
    \item The beginning of \autoref{s-factor} is also included.
    Here, the usefulness of binary quadratic forms for factoring is realised.
    The key result that the previous chapters have built towards is provided in \autoref{s-factor-amb};
    ``ambiguous forms'' immediately provide a factorisation of the discriminant.
\end{itemize}

The tasks for the remainder of the project are to finish formalising the derivations in \autoref{s-comp},
...
This will require adding to the content of \autoref{s-forms}, and possibly adjusting the arguments in \autoref{s-comp}.
...
This can easily be shown to happen in finite time, as $|b|$ decreases with each repetition, so there can be at most $|b|$ repetitions.
...
\todo{convert to pseudocode}

It remains to prove that this is truly a canonical representative.

\vspace{2mm}
\begin{theorem}
    Every positive definite form is properly equivalent to exactly one reduced form. 
    That is, a form satisfying $|b| \leq a \leq c$ and $b \geq 0$ if $a = |b|$ or $a = c$.
\end{theorem}
...
\begin{proof}
    ...
    In \autoref{s-forms-eq}, we showed that the result of this acting on our form is
    $$\left[ap^2 + bpr + cr^2 \right] x^2
        + \left[ 2apq + b(ps + qr) + crs \right] xy
        + \left[ aq^2 + bqs + cs^2 \right] y^2$$
    ...
    \begin{align*}
        ap^2 + bpr + cr^2 &\leq ap^2 - a|pr| + ar^2 \\
    ...
    Any other reduced form would also have this property, so a reduced form equivalent to this one must also have $a$ as its first coefficient.

    So we must now consider when the above inequality acheives equality.
    First note that we can only achieve equality if $pr = 0$, as $b > -a$ in a reduced form.
    Hence, we must have $p^2 + r^2 = 1$. This gives two possible cases
    \begin{gather*}
    ...
    Hence we have
    $$\phi = \pmat{\pm 1 & q \\ 0 & \pm 1} = \pmat{\pm 1 & 0 \\ 0 & \pm 1} T^q$$
    ...
    In the second case, equality is only acheived if $c = a$.
    ...
    This gives
    \begin{align*}
        \phi &= \pmat{0 & \mp 1 \\ \pm 1 & s} \\ 
        &= \pmat{0 & -1 \\ -1 & 0} \pmat{\pm 1 & 0 \\ 0 & \pm 1} \pmat{1 & s \\ 0 & 1} \\
        &= S \cdot \pmat{1 & k \\ 0 & 1} \cdot T^s
    \end{align*}

    As before, we can ignore the action of the negative identity matrix.
    Hence we see that the action of $\phi$ is given by
    $$ST^s : (a, b, c) \mapsto (as^2 + bs + a, - 2as - b, a)$$
...
\emph{In this chapter, we draw from \cite[\S2D-\S3A]{cox}.}

Consider the equiality
...
This result was used by Lagrange for a specific number-theoretic proof, but Legendre sought to generalise results of this kind.
He defined a form $F$ to be the \textbf{composition} of $f$ and $g$ if there exist bilinear forms $B_1$ and $B_2$ such that
\begin{gather*}
    (x, y)g(z, w) = F(B_1(x, y; z, w), B_2(x, y; z, w)) \\
    B_i = a_i xz + b_i xw + c_i yz + d_i yw
\end{gather*}

Legendre was able to show that any two forms of the same discriminant can always be composed, but his formulation was not well-defined as a function, as it was multi-valued.
This was finally rectified by Gauss in his \emph{Disquisitiones Arithmaticae} [\red{cite?}], where he altered Legendre's formulation to obtain a single-valued function.
...
However, as the theory of ideals developed, it became apparent that there exists a bijection between certain ideals of quadratic number fields and the class group of quadratic forms.
This provides us a new and perhaps more intuitive way to derive composition of forms;
we first outline this bijection, then we can leverage the multiplicative structure of ideals to define an analogous operation on forms.
...
\red{It has become apparent that this section has mistakes, having attempted to approach the proof from the opposite direction to usual.
The proof will likely have to be adapted to assume the form of $\mathcal{O}_\Delta$, then prove the correspondence from there.}

As was established in \autoref{s-forms-disc}, a form $(a, b, c$) when considered as a continuous function has zeroes at
$$t = \frac{x}{y} = \frac{b^2 \pm \sqrt\Delta}{2a}$$

From this, we can define a ring
$$\Z\left[\frac{b^2 \pm \sqrt\Delta}{2a}\right] = \Z + \frac{b^2 - \sqrt\Delta}{2a}\Z$$

This can be thought of as a free $\Z$-module of rank 2 or a 2d lattice.
But this isn't quite the formulation that we need - 
the desired behaviour is that these rings would naturally sit inside of a common, larger ring, where they could be extended to ideals and multiplied with one another.
The issue is that the denominator containing $a$ makes this difficult, so let us multiply by $a$.
The result is an isomorphic ring, but when we consider aligning several such rings, this result is much more useful
$$a\Z + \frac{b^2 - \sqrt\Delta}{2}\Z$$

A ring of this form sits inside of the ring
$$\Z\left[\frac{b - \sqrt\Delta}{2}\right] = 
\begin{cases}
    b \equiv 0 \hspace{2mm} \text{(mod 2)} & \Z + \frac{0 - \sqrt\Delta}{2} \\
    b \equiv 1 \hspace{2mm} \text{(mod 2)} & \Z + \frac{1 - \sqrt\Delta}{2}
\end{cases}$$

Noting that $\Delta = b^2 - 4ac$ requires $\Delta \equiv b^2 \hspace{2mm} \text{(mod 4)}$, 
then we also have $\Delta \equiv b \hspace{2mm} \text{(mod 2)}$.
So the rings we defined sit inside
$$\Z + \frac{\Delta - \sqrt\Delta}{2}\Z$$

This depends only on the discriminant, not the particular form, so we will denote the larger ring as $\mathcal{O}_\Delta$.
To ensure that the smaller rings that we started with are truly ideals, 
we will use the same generators to generate an ideal of $\mathcal{O}$ instead of taking them as generators of $\Z$-modules.
This gives a map from forms of discriminant $\Delta$ to ideals of $\mathcal{O}_\Delta$
$$\varphi : (a, b, c) \mapsto \left\langle a, \frac{b - \sqrt\Delta}{2} \right\rangle$$
...
\begin{align*}
    ...
    &= \left\langle a, \frac{2a + b - \sqrt\Delta}{2} \right\rangle \\
    &= \left\langle a, \frac{b - \sqrt\Delta}{2} \right\rangle \\
    &= \varphi \circ f(x, y)
\end{align*}
...
\begin{align*}
    ...
    &= \left\langle c, \frac{-b - \sqrt\Delta}{2} \right\rangle \\
\end{align*}

\todo{we need to introduce ideal equivalence via principal forms (scalar multiplication). How does this arise naturally?}

\todo{formalise/prove the bijection, including the formulation from the other side, taking norms of ideals to get the associated form.}
