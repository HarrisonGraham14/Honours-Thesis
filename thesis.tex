...
Except for this line, it is clear that $g(\tfrac{x}{y})$ agrees with $f(x, y)$ on zeroes and sign.
Hence, consider
\begin{gather*}
    at^2 + bt + c = 0 \\
...
This motivates defining the \textbf{discriminant} of a form $ax^2 + bxy + cy^2$ to be $\Delta = b^2 - 4ac$.
...
Note that the only possible values for a perfect square modulo 4 are 1 and 0, and $\Delta \equiv b^2 \hspace{2mm} \text{(mod 4)}$.
Therefore, we must have 

\vspace{2mm}
\begin{lemma}
    $\Delta$ is equivalent to 1 or 0 modulo 4. 
\end{lemma}
...
This algorithm must terminate in finite time, as $|b|$ decreases with each repetition, so there can be at most $|b|$ repetitions.
...
\begin{proof}
    ...
    Suppose $(a, b, c)$ is a reduced form.
    We claim that $a$ is the minimal coefficient of $x^2$ for all forms in its equivalence class.
    Consider a general transformation
    $$\phi = \pmat{p & q \\ r & s} \in \SL_2(\Z)$$
    In \Cref{s-forms-eq}, we showed that the result of this acting on our form is
    $$\left[ap^2 + bpr + cr^2 \right] x^2
        + \left[ 2apq + b(ps + qr) + crs \right] xy
        + \left[ aq^2 + bqs + cs^2 \right] y^2$$
    We can bound the coefficient of $x^2$ using the fact that $(a, b, c)$ is reduced, hence $c \geq a$ and $b \in (-a, a]$
    \begin{align*}
        ap^2 + bpr + cr^2 &\geq ap^2 - a|pr| + ar^2 \\
        &= a \left[ |p|^2 - |pr| + |r|^2 \right] \\
    ...
    But the determinant of $\phi$ is known to be 1, so the coefficient of $x^2$ is at least $a$.
    By the same argument, any other reduced form $(a', b', c')$ has $a'$ as the minimal leading coefficient among all equivalent forms.
    Hence we see that $a = a'$ --- ie. equivalent reduced forms must have the same leading coefficient.
    
    Now, the inequality above sets a lower bound on the leading coefficient of our new form, which must be $a$ in order for the form to be reduced.
    But the final expression, $a \left[ (|p| - |r|)^2 + |pr| \right]$, is $a$ times some integer.
    The only way that the LHS can be $a$ is if this integer is 1, and if the inequality achieves equality.
    First note that in the first line we used $b > -a \implies bpr \geq -a|pr|$.
    This only achieves equality if $pr = 0$.
    Hence, to achieve equality we require
    \begin{align*}
        a &= a \left[ |p|^2 - |pr| + |r|^2 \right] \\
        \implies 1 &= |p|^2 - |pr| + |r|^2 = p^2 + r^2
    \end{align*}
    ...
    We can infer more information about the coefficients of $\phi$ based on the knowledge that its discriminant is 1.
    \begin{align*}
        \text{det}(\phi) &= ps - qr \\
        1 &= \delta s - 0 \\
        s &= \delta
    ...
    In the second case, letting $r = \delta \in \{1, -1\}$, equality is only acheived if $c = a$ (since we took $cr^2 \geq ar^2$, and $r \neq 0$).
    Again investigating $\phi$, we have
    \begin{align*}
        \text{det}(\phi) &= ps - qr \\
        1 &= 0 - \delta q \\
        q &= - \delta
    ...
    So $s = 0$, in which case $\phi = S$, or $as + b = 0$. 
    In the former case, the action of $\phi$ flips the sign of the second coefficient, which turns a reduced form into one that is no longer reduced 
    (unless $b = 0$, but in this case, $\phi$ acts tivially on the form).
    In the latter case, since $(a, b, a)$ is reduced, we have $a \geq b$ and $|b| \geq 0$.
    We must therefore have $a = b$, $s = -1$.
...
This was finally rectified by Gauss in his \emph{Disquisitiones Arithmaticae} [\red{cite?}], where he refined Legendre's formulation to turn composition of forms into a well-defined function.
...
\todo{remark that we are working with the definition that rings must include multiplicative identities, and that subrings must have the same multiplicative}

\todo{introduce disriminant with mod 4 conditions}
...
\begin{proof}
    ...
    Closure under addition is obvious - as for closure under multiplication, let us show closure of the generators, 1 and $\frac{\Delta + \sqrt\Delta}{2}$, from which closure of all other elements can be inferred.
    ...
    and since $\Delta \equiv 0, 1 \pmod{4}$, [\red{ref}] this is in $\order_\Delta$.
\end{proof}

\begin{lemma}
    A nonzero ideal $I \trianglelefteq \order_\Delta$ is of the form
    $$I = l\Z + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right)\Z$$
\end{lemma}

\begin{proof}
    Consider a nonzero element $x + \frac{\Delta + \sqrt\Delta}{2}y \in I$. We have
    $$x + \frac{\Delta - \sqrt\Delta}{2}y = x + y\Delta - \frac{\Delta + \sqrt\Delta}{2}y \in \order_\Delta$$

    so, since $I$ is an ideal of $\order_\Delta$
    \begin{align*}
        \left( x + \frac{\Delta + \sqrt\Delta}{2}y \right) \left( x + \frac{\Delta - \sqrt\Delta}{2}y \right)
        &= \left( x + \frac{\Delta}{2}y \right)^2 -  \left( \frac{\sqrt\Delta}{2}y \right)^2 \\
        &= x + \Delta xy + \frac{\Delta^2 y^2}{4} - \frac{\Delta y^2}{4} \\
        &= x + \Delta xy + \frac{\Delta(\Delta - 1)y^2}{4} \in I
    \end{align*}

    This is an integer, so there is at least one integer in $I$.
    Multiplying this by $\frac{\Delta + \sqrt\Delta}{2} \in \order_\Delta$, we get that there is at least one element in $I$ with a nonzero $\frac{\Delta + \sqrt\Delta}{2}$ component.
    Let $l$ be the minimum positive integer in $I$, and choose $m$ and $n$ such that $m + n\frac{\Delta + \sqrt\Delta}{2} \in I$, where $|n| \neq 0$ is minimum.
    Every integer $z \in I \cap \Z$ must satisfy $l | z$, or else $\gcd(l, z) \in \Z$ would cause a contradiction with the choice of $l$.
    For any $x + \frac{\Delta + \sqrt\Delta}{2}y \in I$ we must also have $n | y$, or else the choice of $n$ would be contradicted.
    Letting $y = y_0 n$, we can write
    \begin{align*}
        x + \frac{\Delta + \sqrt\Delta}{2}y &= x + \frac{\Delta + \sqrt\Delta}{2}y_0 n + y_0 m - y_0 m \\
        &= x - y_0 m + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) y_0
    \end{align*}
    and $(m + n\frac{\Delta + \sqrt\Delta}{2}) y_0 \in I$, so $x - y_0 m$ must also be in $I$, which means $l | x - y_0 m$.
    Hence $x + \frac{\Delta + \sqrt\Delta}{2}y \in l\Z + (m + n\frac{\Delta + \sqrt\Delta}{2}\Z)$, so $I = l\Z + (m + n\frac{\Delta + \sqrt\Delta}{2}\Z)$.
\end{proof}

Finally, we must define the norm of an element. 
...
$$N(x + y\sqrt\Delta) = (x + y\sqrt\Delta) \cdot (x - y\sqrt\Delta).$$
Let us consider the norm of a general element of $I \trianglelefteq \order_\Delta$. We have
\begin{align*}
    &\hspace{-5mm} N\left(lx + \left(m + n\frac{\Delta + \sqrt\Delta}{2}\right)y\right) \\
    &= \left( lx + my + n\frac{\Delta}{2}y + n\frac{\sqrt\Delta}{2}y \right) \cdot \left( lx + my + n\frac{\Delta}{2}y - n\frac{\sqrt\Delta}{2}y \right) \\
    &= \left( lx + my + n\frac{\Delta}{2}y \right)^2 - \left(n\frac{\sqrt\Delta}{2}y \right)^2 \\
    &= l^2x^2 + m^2y^2 + n^2\frac{\Delta^2}{4}y^2 + 2lmxy + ln\Delta xy + mn\Delta y^2 - n^2\frac{\Delta}{4}y^2 \\
    &= l^2x^2 + l(2m + n\Delta)xy + \left(m^2 + mn\Delta + n^2\frac{\Delta^2 - \Delta}{4}\right)y^2 \tag{\theequation}\label{eq-ideal-norm-form}
\end{align*}
...
First, note that the discriminant of this form found in \eqref{eq-ideal-norm-form} is given by
\begin{align*}
    ...
    &= (ln)^2\Delta
\end{align*}
By \todo{add theorem to discriminant section}, if we can divide the form by $ln$, we will arrive at a form of discriminant $\Delta$.
Fortunately, the fact that $I = l\Z + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right)\Z$ is an ideal of $\order_\Delta$ constrains $l, m, n$ to certain divisibility conditions.
First, consider
\begin{align*}
    l \cdot \frac{\Delta + \sqrt\Delta}{2} &= \left( n\frac{\Delta + \sqrt\Delta}{2} \right) \frac{l}{n} \\
    &= \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \frac{l}{n} - \frac{ml}{n} \\
    &= \frac{m}{n}l + \frac{l}{n}\left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \in I
\end{align*}
Hence we see that $\frac{m}{n}$ and $\frac{l}{n}$ are integers.
Next, consider
\begin{align*}
    &\hspace{-5mm} \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \cdot \left(\left(\frac{m}{n} + \Delta\right) - \frac{\Delta + \sqrt\Delta}{2} \right) \\
    &= \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \cdot \left(\frac{m}{n} + \frac{\Delta - \sqrt\Delta}{2} \right) \\
    &= \frac{1}{n} \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \cdot \left(m + n\frac{\Delta - \sqrt\Delta}{2} \right) \\
    &= \frac{1}{n} \left[ \left( m + n\frac{\Delta}{2} \right)^2 - \left( n\frac{\sqrt\Delta}{2} \right)^2 \right] \\
    &= \frac{1}{n} \left[ m^2 + 2mn\Delta + n^2\frac{\Delta^2}{4} - n^2\frac{\Delta}{4} \right] \\
    &= \frac{1}{ln}\left( m^2 + 2mn\Delta + n^2\frac{\Delta^2 - \Delta}{4} \right) l \in I
\end{align*}
Hence, the coefficient of $l$ is an integer. Using these results, consider the form in \eqref{eq-ideal-norm-form} when divided by $ln$;
$$\frac{l}{n}x^2 + \left(2\frac{m}{n} + \Delta\right)xy + \frac{1}{ln}\left(m^2 + mn\Delta + n^2\frac{\Delta^2 - \Delta}{4}\right)y^2$$
We have shown each coefficient to be an integer, hence this is a binary quadratic form of discriminant $\Delta$.

We now have a map from ideals of $\order_\Delta$ to forms of discriminant $\Delta$.
$$\left\langle l, m + n\frac{\Delta + \sqrt\Delta}{2} \right\rangle \mapsto \left( \frac{l}{n}, 2\frac{m}{n} + \Delta, \frac{1}{ln}\left(m^2 + mn\Delta + n^2\frac{\Delta^2 - \Delta}{4}\right) \right)$$
\todo{remark that this sends ideals that vary by constant multiples to the same form, so we must define equivalence of ideals up to multiplication by elements of $K$.}
To establish a bijection, we must show that its inverse exists and is sensible.
To do this, we must first know what conditions on $l, m, n \in \Z$ are necessary for
$$I = l\Z + \left( m + n \frac{\Delta + \sqrt\Delta}{2} \right) \Z$$
to be an ideal of $\order_\Delta$. Certainly $I$ is a subset of $\order_\Delta$ which is closed under addition and multiplication by 1.
Under multiplication by the other basis element of $\order_\Delta$, we have
$$l \cdot \frac{\Delta + \sqrt\Delta}{2} = \frac{m}{n}l + \frac{l}{n}\left( m + n\frac{\Delta + \sqrt\Delta}{2} \right)$$
and
\begin{align*}
    ...
    &= \left( \frac{m}{n} + \Delta \right) \left(m + n\frac{\Delta + \sqrt\Delta}{2} \right) - \frac{1}{ln}\left( m^2 + mn\Delta + n^2 \frac{\Delta - \Delta^2}{4} \right)l
\end{align*}
Hence we see that the necessary conditions for $I \trianglelefteq \order_\Delta$ are
\begin{itemize}
    \item $n | m$
    \item $n | l$
    \item $ln | \left( m^2 + mn\Delta + n^2 \frac{\Delta - \Delta^2}{4} \right)$
\end{itemize}

\todo{finish inverse map}
...
$$I_f = \left\langle a, \frac{b - \sqrt\Delta}{2} \right\rangle, \hspace{5mm} I_g = \left\langle a', \frac{b' - \sqrt\Delta}{2} \right\rangle$$

The product of these ideals is the ideal generated by the pairwise products of their generators
$$I_f \cdot I_g = \left\langle aa', a\frac{b' - \sqrt\Delta}{2}, a'\frac{b - \sqrt\Delta}{2}, \frac{bb' + \Delta - (b + b')\sqrt\Delta}{4} \right\rangle$$

It is known that such an ideal can be expressed using at most two generators \cite[Thm.\,5.9]{stewart}.
This can be easily found computationally using an algorithm for ideal reduction such as is provided by Cohen \cite[Ch.\,2.4.2]{cohen}.
However, we seek to find a more general closed form.

This task would be made much more simple if we could reduce the number of variables.
To achieve this, let us assume that we can rewrite the forms such that the $b$ term is equal - this isn't as large of an assumption as it might seem.
Certainly, if the $a$ terms of either form don't share a common factor, then by applying the correct number of $T$ transformations to each form, their $b$ terms can be made to match.

Hence, let us suppose that the forms can be expressed as
$$f \equiv (a_1, B, c_1), \hspace{5mm} g \equiv (a_2, B, c_2)$$

Using the fact that these forms share the same discriminant
\begin{gather*}
    \Delta = B^2 - 4a_1c_1 = B^2 - 4a_2c_2 \\
    \frac{\Delta - B^2}{4} = a_1c_1 = a_2c_2 \\
    \frac{\Delta - B^2}{4a_1a_2} = \frac{c_1}{a_2} = \frac{c_2}{a_1} \rnote{\text{$\in \Z$ if $(a_1, a_2) = 1$}}
\end{gather*}

So by letting $C = \tfrac{\Delta - B^2}{4a_1a_2}$, we can express the forms as
$$f \equiv (a_1, B, a_2 C), \hspace{5mm} g \equiv (a_2, B, a_1 C)$$

This gives the product of ideals to be
$$I_f \cdot I_g = \left\langle a_1a_2, a_1\frac{B - \sqrt\Delta}{2}, a_2\frac{B - \sqrt\Delta}{2}, \frac{B^2 + \Delta - 2B\sqrt\Delta}{4} \right\rangle$$

This final term is
\begin{align*}
    \frac{B^2 + \Delta - 2B\sqrt\Delta}{4} &= \frac{B^2 + B^2 - 4a_1a_2C - 2B\sqrt\Delta}{4} \\
    &= B\frac{B - \sqrt\Delta}{2} - a_1a_2C
\end{align*}

This reduces to
$$I_f \cdot I_g = \left\langle a_1a_2, \hspace{2mm} \text{gcd}(a_1, a_2, B) \frac{B - \sqrt\Delta}{2}\right\rangle$$

\todo{declare gcd(a1, a2, B) = 1 as f is primitive}

\todo{express $B$, $a_1$, and $a_2$ as functions of $a, a', b, b'$. Consider the case when a and a' have a common divisor. Can c/c' be used instead? Can this always be adapted to work?}
