...
Note that the only possible values for a perfect square modulo 4 are 1 and 0, and $\disc \equiv b^2 \hspace{2mm} \text{(mod 4)}$.
Therefore, we must have 

\begin{remark}
    $\disc$ is equivalent to 1 or 0 modulo 4. 
\end{remark}
...
\red{Do I add a proof that equivalent forms represent the same integers?}
...
\begin{theorem}
    Exactly one form exists in a given equivalence class of positive definite forms satisfying $|b| \leq a \leq c$ and $b \geq 0$ if $a = |b|$ or $a = c$.
\end{theorem}

\begin{proof}
    We adapt the proof of Cohen \cite[Prop.\,5.3.3]{cohen}.
    \todo{adapt proof}
\end{proof}
...
This depends only on the discriminant, not the particular form, so we will denote the larger ring as $\mathcal{O}_\disc$.
...
This gives a map from forms of discriminant $\disc$ to ideals of $\mathcal{O}_\disc$
$$\varphi : (a, b, c) \mapsto \left\langle a, \frac{b - \sqrt\disc}{2} \right\rangle$$
...
\begin{align*}
    ...
    &= \left\langle a, \frac{2a + b - \sqrt\disc}{2} \right\rangle \\
    &= \left\langle a, \frac{b - \sqrt\disc}{2} \right\rangle \\
    &= \varphi \circ f(x, y)
\end{align*}
...
\begin{align*}
    ...
    &= \left\langle c, \frac{-b - \sqrt\disc}{2} \right\rangle \\
\end{align*}
...
Let us begin by considering forms $f = (a, b, c)$ and $g = (a', b', c')$, both of discriminant $\disc$.
These correspond to ideals of $\mathcal{O}_\disc$
$$I_f = \left\langle a, \frac{b - \sqrt\disc}{2} \right\rangle, \hspace{5mm} I_g = \left\langle a', \frac{b' - \sqrt\disc}{2} \right\rangle$$

The product of these ideals is the ideal generated by the pairwise products of their generators
$$I_f \cdot I_g = \left\langle aa', a\frac{b' - \sqrt\disc}{2}, a'\frac{b - \sqrt\disc}{2}, \frac{bb' + \disc - (b + b')\sqrt\disc}{4} \right\rangle$$
...
\begin{gather*}
    \disc = B^2 - 4a_1c_1 = B^2 - 4a_2c_2 \\
    \frac{\disc - B^2}{4} = a_1c_1 = a_2c_2 \\
    \frac{\disc - B^2}{4a_1a_2} = \frac{c_1}{a_2} = \frac{c_2}{a_1} \rnote{\text{$\in \Z$ if $(a_1, a_2) = 1$}}
\end{gather*}

So by letting $C = \tfrac{\disc - B^2}{4a_1a_2}$, we can express the forms as
$$f \equiv (a_1, B, a_2 C), \hspace{5mm} g \equiv (a_2, B, a_1 C)$$

This gives the product of ideals to be
$$I_f \cdot I_g = \left\langle a_1a_2, a_1\frac{B - \sqrt\disc}{2}, a_2\frac{B - \sqrt\disc}{2}, \frac{B^2 + \disc - 2B\sqrt\disc}{4} \right\rangle$$

This final term is
\begin{align*}
    \frac{B^2 + \disc - 2B\sqrt\disc}{4} &= \frac{B^2 + B^2 - 4a_1a_2C - 2B\sqrt\disc}{4} \\
    &= B\frac{B - \sqrt\disc}{2} - a_1a_2C
\end{align*}

This reduces to
$$I_f \cdot I_g = \left\langle a_1a_2, \hspace{2mm} \text{gcd}(a_1, a_2, B) \frac{B - \sqrt\disc}{2}\right\rangle$$
