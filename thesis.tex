...
However, note that $f$ can be extended to a continuous function over $\R^2$ and that $f(x, 0) = x^2$, which can only be zero when $x$ is zero.
...
We call forms with $b^2 -4ac < 0$ \textbf{positive definite} and \textbf{negative definite}, depending on whether they output positive or negative values.
...
Similar reasoning shows that forms with $b^2 -4ac = 0$ are strictly non-negative or non-positive, but such forms are of little intereest.
...
\begin{lemma}\label{lemma-discriminant-under-multiple}
    If the discriminant of the form $(a, b, c)$ is $\Delta$, then the discriminant of the form $(an, bn, cn)$ is $\Delta n^2$.
\end{lemma}

\begin{proof}
    The discriminant of $(an, bn, cn)$ is given By
    $$(bn)^2 - 4(an)(cn) = b^2n^2 - 4acn^2 = \Delta n^2.$$
\end{proof}
...
\begin{proof}
    ...
    We can therefore see that the choice of $l$ is unique up to sign.

    Likewise, choose $m + n\frac{\Delta + \sqrt\Delta}{2} \in I$ such that $n > 0$ is minimum.
    For any $x + \frac{\Delta + \sqrt\Delta}{2}y \in I$ we must also have $n | y$, or else a linear combination of elements would yield a result whose coefficient of $\frac{\Delta + \sqrt\Delta}{2}$ would be $\gcd(y, n)$, contradicting the choice of $n$.
...
Note that $l$ and $n$ are uniquely determined, while the choice of $m$ is only determined up to adding multiples of $l$.
...
\begin{align*}
    ...
    &= (ln)^2\Delta.
\end{align*}
By \Cref{lemma-discriminant-under-multiple}, if we can divide the form by $ln$, we will arrive at a form of discriminant $\Delta$.
Since we have chosen $l, m, n$ such that $I = l\Z + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right)\Z$ is an ideal of $\order_\Delta$, there are certain divisibility conditions which will prove helpful.
...
\begin{lemma}\label{lemma-lattice-divisibility-conditions}
    ...
    \begin{enumerate}
        \item $n | m$
        \item $n | l$
        \item $l | \frac{m^2}{n} + m\Delta + n \frac{\Delta - \Delta^2}{4}.$
    \end{enumerate}
\end{lemma}

\begin{proof}
    First, we consider the necessary conditions for $I \trianglelefteq \order_\Delta$. 
    For $I$ to be an ideal, it must be closed over addition and under multiplication by elements of $\order_\Delta$.
    Closure under addition is obvious, since $I$ is a $\Z$-module.
    For closure under multiplication, we need only test the generators of $I$ multiplied by the generators of $\order_\Delta$, from which closure of all multiples can be inferred.
    The generators of $I$ are $l$ and $m + n\frac{\Delta + \sqrt\Delta}{2}$, and the generators of $\order_\Delta$ are $1$ and $\frac{\Delta + \sqrt\Delta}{2}$.
    Closure under multiplication by 1 is obvious, so we have two conditions remaining to check.
    First,
    \begin{align*}
        l \cdot \frac{\Delta + \sqrt\Delta}{2} &= \left( n\frac{\Delta + \sqrt\Delta}{2} \right) \frac{l}{n} \\
        &= - \frac{m}{n}l + \frac{l}{n}\left( m + n\frac{\Delta + \sqrt\Delta}{2} \right).
    \end{align*}
    Since $I$ is a free $\Q$-module, representation of elements as linear combinations of basis elements are unique.
    Since this is the only way to write the result as a combination of basis elements, for this to be in $I$ we require that the coefficients be integers.
    Hence, we require $n | m$ and $n | l$.

    The other condition for closure under multiplication comes from
    \begin{align*}
    ...
    \end{align*}
    Again, for this to be in $I$, we require the coefficients on the generators to be integers.
    Hence we have the added condition $l | \frac{m^2}{n} + m\Delta + n \frac{\Delta - \Delta^2}{4}$.

    To prove that these conditions are sufficient, suppose that we have chosen $l, m, n$ satisfying the divisibility conditions.
    We have already shown that these are the necessary conditions for $l\Z + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right)\Z$ to satisty closure under multiplication by elements of $\order_\Delta$, so we are done.
\end{proof}

Using this result, consider the form in \eqref{eq-ideal-norm-form} when divided by $ln$;
$$\frac{l}{n}x^2 + \left(\frac{2m}{n} + \Delta\right)xy + \frac{1}{l}\left(\frac{m^2}{n} + m\Delta + n\frac{\Delta^2 - \Delta}{4}\right)y^2.$$
...
We can therefore define a map from ideals of $\order_\Delta$ to forms of discriminant $\Delta$.
$$l\Z + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \Z \mapsto \left( \frac{l}{n}, \frac{2m}{n} + \Delta, \frac{1}{l}\left(\frac{m^2}{n} + m\Delta + n\frac{\Delta^2 - \Delta}{4}\right) \right).$$
Recall that for a given ideal, $m$ is only uniquely determined up to adding multiples of $l$.
...
Given an ideal $I = l\Z + \left( m + n\frac{\Delta + \sqrt\Delta}{2} \right) \Z$, we can also write 
$$I = l\Z + \left( (m + kl) + n\frac{\Delta + \sqrt\Delta}{2} \right) \Z$$
where $k \in \Z$. 
Our map sends this ideal to
\begin{align*}
...
To show that this map is a bijection, we must find the inverse map, and show that it respects the equivalence classes that we have defined.
We want a map
\begin{align*}
    (a, b, c) &= \left( \frac{l}{n}, \frac{2m}{n} + \Delta, \frac{1}{l}\left(\frac{m^2}{n} + m\Delta + n\frac{\Delta^2 - \Delta}{4}\right) \right) \\
...
Choosing $n = 1$ immediately satisfies the conditions $n | m$ and $n | l$, and it immediately forces $l = a$ and $m = \frac{b - \Delta}{2}$ (which is always an integer, as $b \equiv \Delta \pmod{4}$).
...
$$(a, b, c) \mapsto a\Z + \left( \frac{b - \Delta}{2} + \frac{\Delta + \sqrt\Delta}{2} \right) \Z = \left\langle a, \frac{b + \sqrt\Delta}{2} \right\rangle.$$
...
Using the fact that these forms share the same discriminant, we have
\begin{gather*}
